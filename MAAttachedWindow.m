                        ...
                        atDistance:(float)distance
{
    ...
    if ((self = [super initWithContentRect:contentRect 
		...
		imgView = [[NSImageView alloc] initWithFrame:NSZeroRect];
...
- (void)_updateBackground
{
	...
	NSImage* bg = [self _backgroundImage];
	imgView.frame = NSMakeRect(0,0,bg.size.width, bg.size.height);
	[imgView setImage: bg];
    if ([self isVisible]) {
        [self display];
...
- (NSColor *)_backgroundColorPatternImage
{
    ...
    return [NSColor colorWithPatternImage:[self _backgroundImage]];
}
